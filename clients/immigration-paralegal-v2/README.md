...
- **Draft 3 (current)**: fully new palette — warm ivory/plum/teal, no reused clay or forest. New centerpiece — a pinned stacked-panel reveal using `clip-path` wipes (4 full-viewport panels stacked with `z-index`, each revealing over the last as the section is pinned and scrolled). Two real bugs found and fixed during this draft, both from the same root cause:
  - Panels lacked `z-index` stacking, so mid-transition both the outgoing and incoming panel's text rendered simultaneously, overlapping — fixed by giving each panel an explicit `z-index` and opaque background.
  - **Kallon caught a real UX bug after seeing the finished build**: the section appeared to "start at panel 2 instead of panel 1." Root cause: `.stack-panel:first-child{clip-path:inset(0 0 0 0)}` never matched anything, because `.stack-dots` (a sibling `<div>`) is the actual first child of `.stack-sticky` — panel 1 ("Consultation") was never given its default-visible state, so on entering the pinned section nothing was visible until GSAP's timeline revealed panel 2. Same root cause also silently shifted every panel's `z-index` by one via the equivalent `:nth-child()` rules (panel 4 got no z-index at all). Fixed by switching both from structural pseudo-selectors to the already-present `data-panel` attribute / an explicit `.first` class — robust regardless of sibling order. Also added a proper hold+transition timeline structure (each panel now holds for a real scroll distance before wiping to the next, instead of the wipe starting almost immediately).
